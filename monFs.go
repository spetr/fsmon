...
func monFsUpdate(mountpoint, name string) {
	...
	t := time.Now().Unix()
	metrics[0] = zabbix.NewMetric(conf.Zabbix.Hostname, "fsmon.discovery", fmt.Sprintf("[{\"{#NAME}\": \"%s\"}]", name), false, t)
	...
	for {
		...
		if len(conf.Zabbix.Servers) > 0 {
			...
			t := time.Now().Unix()
			metrics[0] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.mkdir.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsMkdir)/1000/1000), false, t)
			metrics[1] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.list.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsList)/1000/1000), false, t)
			metrics[2] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.create.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsCreate)/1000/1000), false, t)
			metrics[3] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.open.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsOpen)/1000/1000), false, t)
			metrics[4] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.write.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsWrite)/1000/1000), false, t)
			metrics[5] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.sync.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsSync)/1000/1000), false, t)
			metrics[6] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.read.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsRead)/1000/1000), false, t)
			metrics[7] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.close.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsClose)/1000/1000), false, t)
			metrics[8] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.stat.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsStat)/1000/1000), false, t)
			metrics[9] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.statnx.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsStatNx)/1000/1000), false, t)
			metrics[10] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.delete.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsDelete)/1000/1000), false, t)
			metrics[11] = zabbix.NewMetric(conf.Zabbix.Hostname, fmt.Sprintf("fsmon.rmdir.[%s]", name), fmt.Sprintf("%.6f", float64(timeMonFsRmdir)/1000/1000), false, t)
			for i := range conf.Zabbix.Servers {
